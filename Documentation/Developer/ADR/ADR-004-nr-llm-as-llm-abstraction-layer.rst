...
The extension needs to call an LLM API. Multiple providers are relevant (OpenAI,
Anthropic Claude, Google Gemini, Ollama), each with different SDKs,
authentication schemes, and capability sets (vision, native document handling,
tool calling).
...
-   **``netresearch/nr-llm``**: Existing Netresearch TYPO3 extension providing a
    ...
    interfaces (``DocumentCapableInterface``, ``VisionCapableInterface``).

Decision
========

Use ``netresearch/nr-llm`` as the sole LLM integration point. The extension
references an ``nr-llm`` Task record (configured by the TYPO3 administrator) and
delegates all LLM calls through its interface.
